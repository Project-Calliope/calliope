...
def test_api_handler_creation():
    ...
    assert isinstance(handler, APIHandler), "L'objet créé n'est pas une instance de APIHandler"

def test_api_handler_attributes():
    handler = APIHandler()

    assert hasattr(handler, 'data_manager'), "L'APIHandler ne possède pas l'attribut 'data_manager'"
